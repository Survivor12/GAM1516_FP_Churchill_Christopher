...
public class MarioMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (MarioMovement.IsClose(rigidbody.linearVelocity.x, 0.0f, 0.01f) && linearVelocity.x != 0.0f)
        {
            linearVelocity.x = 0.0f;
        }

        // Set the y velocity to match the rigid body's y velocity
        linearVelocity.y = rigidbody.linearVelocity.y;

        // Detect when mario is on the ground or falling
        if (MarioMovement.IsClose(linearVelocity.y, 0.0f, 0.02f) && lastLinearVelocity.y < 0.0f)
        {
    ...
    private void FixedUpdate()
    {
        ...
        rigidbody.linearVelocity = linearVelocity * localTimeScale;

        // Check if the jump has reached the apex
        if (checkJumpApex && rigidbody.linearVelocity.y <= 0.0f)
        {
            checkJumpApex = false;
    ...
    public bool IsSkidding()
    {
        ...
        float direction = Mathf.Sign(linearVelocity.x);
    ...
    private void HorizontalMovement()
    {
        ...
        {
            // Ground physics
            if (Mathf.Abs(linearVelocity.x) < minWalkSpeed)
            {
                linearVelocity.x = 0.0f;

                if (marioController.IsDuckPressed() == false)
                {
                    linearVelocity.x += inputX * minWalkSpeed * localTimeScale;
                }
            }
            else if (Mathf.Abs(linearVelocity.x) >= minWalkSpeed)
            {
                if (direction == 1.0f) 
                {
                    if (inputX > 0.0f && marioController.IsDuckPressed() == false)
                    {
                        linearVelocity.x += acceleration * Time.deltaTime * localTimeScale;
                    }
                    else if (inputX < 0.0f && marioController.IsDuckPressed() == false)
                    {
                        linearVelocity.x -= decelerationSkid * Time.deltaTime * localTimeScale;
                    }
                    else
                    {
                        linearVelocity.x -= deceleration * Time.deltaTime * localTimeScale;
                    }
                }
        ...
        else
        {
            ...
            if (inputX > 0.0f)
            {
                linearVelocity.x += acceleration * airControl * Time.deltaTime * localTimeScale;
            }
            else if (inputX < 0.0f)
            {
                linearVelocity.x -= acceleration * airControl * Time.deltaTime * localTimeScale;
        ...
        if (linearVelocity.x >= maxWalkSpeed)
            linearVelocity.x = maxWalkSpeed;
